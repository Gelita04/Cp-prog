...
{
    public static int N_Deposits(int[] posiciones_depositos, int total_trabajadores, int[,] combustible)
    {
    ...
    public static void N_Deposits(bool[] mask, int[] posiciones_depositos, int id_deposito_actual, int total_trabajadores, int id_trabajador_actual, int[,] combustible, ref int result, int posicion_actual, int visitadas, int combustible_total)
    {
        //OOOOOOOOOOOOOOOJJJJJJJJJJOOOOOOOOOOOOOOOOOO PROBAR HACER EL PROBLEMA DE ALIMENTANDO LEONES PUDIENDO IR AL DEPOSITO N VECES EN VEZ DE UNA SOLA VEZ SOLAMENTE y hacerlo agreagando limite de capacidad
        ...
        {
            if (!posiciones_depositos.All(x => x != posicion_a_viajar))
                continue;
        ...
        {
            N_Deposits(mask, posiciones_depositos, id_deposito_actual + 1, total_trabajadores, id_trabajador_actual + 1, combustible, ref result, posiciones_depositos[id_deposito_actual + 1], visitadas, combustible_total + combustible[posicion_actual, posiciones_depositos[id_deposito_actual]]);
        }
    }
}
